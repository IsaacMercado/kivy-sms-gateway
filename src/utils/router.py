...
class MixinAppRouter:
    ...
    def on_route(self, instance, route):
        Logger.info("AppRouter: Route is '%s'", route)
    ...
    def init_routes(self):
        for key in dir(self):
            ...
            for rule, regex, url_parts in func.router_rules:
                ...
                self.add_route(rule, func, regex=regex,
                               url_parts=url_parts, options=options)

    def add_route(self, rule, func, regex=None, url_parts=None, options=None):
        if regex is None:
            url_parts, regex = regex_rule(rule)
        Logger.debug("%s: Add route %s", self.__class__.__name__, rule)
        url_parts, regex = regex_rule(rule)
